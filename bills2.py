...
def cartdetails(tkindex,delframe):
    ...
    for i in recordcart:
        if str(i[0])==u1:
            ...
            l=l+1
    l=l+1
    (Button(frame12,text="Confirm Order",command=lambda: conforder(flag,total))).grid(row=l,column=0,columnspan=m+1,pady=(15,5))
    newc.commit()
    newc.close()
...
def listproduct(categories):
    ...
    for j in range(count):    
        ...
        (Label(frame200,text="   ")).grid(row=q,column=r)
        r=r+1
        (Label(frame200,text="Rs."+str(pcost_list[j]))).grid(row=q,column=r,pady=(0,10))
        r=r+1
        (Label(frame200,text="   ")).grid(row=q,column=r)
        r=r+1
...
def start():
    ...
    p=Label(frame,text="Welcome to ABC Billing System")
